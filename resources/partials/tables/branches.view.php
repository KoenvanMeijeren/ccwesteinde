                    ...
                    <td class="text-center" width="10%">
                        <div class="pull-left">
                            <button type="button" class="btn btn-primary" data-toggle="tooltip" data-placement="top"
                                    ...
                                    onclick='window.location.href="/admin/branch/<?= $row->entity_ID ?? 0 ?>/edit"'>
                                <i class="fa fa-pencil-square-o" aria-hidden="true"></i>
                            </button>
                        </div>

                        <form class="pull-right" method="post" action="/admin/branch/<?= $row->entity_ID ?? 0 ?>/delete">
                            <button type="submit" class="btn btn-danger" data-toggle="tooltip" data-placement="top"
                                    title="<?= Translation::get('delete') ?>"
                                    onclick="return confirm('<?= Translation::get('delete_branch_confirmation_message') ?>')">
...
<?php else : ?>
    <p><?= Translation::get('no_branches_were_found_message') ?></p>
<?php endif; ?>