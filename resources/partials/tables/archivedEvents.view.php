...
if (isset($keys) && !empty($keys) && isset($rows) && !empty($rows)) : ?>
    <div class="table-responsive">
        <table id="table1" class="table table-light table-hover table-striped text-nowrap">
            <thead>
            <tr>
            ...
            <?php foreach ($rows as $row) : ?>
                <tr>
                    <th scope="row"><?= htmlspecialchars_decode($row->event_title ?? '-') ?></th>
                    ...
                    <td><?= htmlspecialchars_decode($row->event_location ?? '-') ?></td>
                    <td><?= intval($row->event_maximum_persons ?? 0) ?></td>
                    <td><?= intval($row->quantitySignUps ?? 0) ?></td>
                    <td>
                        <div class="row">
                            <form class="m-1" method="post" action="event/<?= $row->event_ID ?? 0 ?>/recover">
                                <button type="submit" class="btn btn-success" data-toggle="tooltip" data-placement="top"
                                        title="<?= Translation::get('recover') ?>"
                                        onclick="return confirm('<?= Translation::get('recover_archive_event_confirmation_message') ?>')">
                                    <?= Translation::get('recover') ?>
                                </button>
                            </form>
                            <div class="m-1">
